...
@router.callback_query(lambda call: call.data in kb.name_button_count_day.values() and "back" not in call.data)
async def ass(call: CallbackQuery, state: FSMContext):
    id = call.from_user.id
    user = users.get(id)
    await state.set_state(States.contract)
    await state.update_data(count_day=int(call.data))
...
@router.callback_query(States.contract,
                       lambda call: call.data in kb.name_button_deposit_contract.values() and "back" not in call.data)
async def ass(call: CallbackQuery, state: FSMContext):
    id = call.from_user.id
    user = users.get(id)
    ...
    if deposit <= user.balance + user.bonus_account:
        ...
        await EditMessageText(chat_id=id,
                              ...
                              text=get_tmp("templates/text_by_confirm_contract.md", id=len(contracts)+1,
                                           count_day=count_day, deposit=deposit),
